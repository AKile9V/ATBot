...
class Commands(commands.Cog):
    ...
    async def manage_own_message(self, ctx, message):
        ...
        for emoji_role in bedne_info:
            final_emoji = get(ctx.guild.emojis,
                              name=emoji_role["emoji"]) or emoji_role["emoji"]
            emoji_role_string = emoji_role_string + additional_games_emoji_template_message.format(
                final_emoji, ctx.guild.get_role(int(emoji_role["role"])))
        ...
        choose_bedna_igra_embed.add_field(
            name="", value=new_message, inline=True)
    ...
    async def bedneigre(self, ctx):
        only_officers(ctx)
        bedne_igre_embed = discord.Embed(
            title=additional_games_title_message, colour=discord.Color.dark_green())
        bedne_igre_embed.add_field(
            name="", value=additional_games_message, inline=True)
    ...
    async def wowtoken(self, ctx):
        ...
        wow_token_embed.add_field(
            name="Trenutna cena wow tokena: ", value=info, inline=True)
    ...
    async def rolemessages(self, ctx):
        ...
        roles_embed = discord.Embed(
            title=str(role_note), colour=discord.Color.dark_green())
        ...
        values = str(albino_rola) + " za albino rolu, " + str(albino_friend_rola) + \
            " za alibno friend rolu, " + str(raider_rola) + " za raider rolu"
    ...
    async def poll(self, ctx, question, *options: str):
        ...
        embed = discord.Embed(title=question, description=''.join(
            description), colour=discord.Color.dark_green())
        ...
        for member in guild.members:
            for user in user_info:
                if user['name'] == member.name:
                    ...
                    writer.writerow([[str(user['name'])], [str(member.created_at)], [str(member.joined_at)], [
                                    user['level'] >= 1], [str(user['level'])], [str(user['xp'])], [all_roles]])
